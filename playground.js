document.addEventListener('DOMContentLoaded', () => {
  ...
  async function fetchList(){
    ...
    try{
      const res = await fetch('/api/playground', { headers: { Authorization: 'Bearer ' + token }});
  ...
  async function createItem(){
    ...
    try{
      const res = await fetch('/api/playground', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + token }, body: JSON.stringify({ name, data }) });
  ...
  async function deleteItem(id){
    ...
    try{
      const res = await fetch('/api/playground/' + id, { method: 'DELETE', headers: { Authorization: 'Bearer ' + token }});
